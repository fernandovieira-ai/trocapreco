...
exports.fazerLogin = async (req, res) => {
  ...
  const user = await db.query_trocaprecos(`
    ...
    WHERE
      nom_usuario = $1
      AND senha = $2
    AND ind_ativo = 'S'`, [nom_usuario, senha]);
...
exports.alterarSenha = async (req, res) => {
  ...
  try {
    ...
    const empresas = await db.query_trocaprecos("update tab_usuario set senha = $1 where cod_usuario = $2", [senha, cod_usuario]);
...
exports.novoUsuario = async (req, res) => {
  ...
  const { nom_usuario, senha, schema_base, des_rede, img_rede, ind_aprova_negociacao } = req.body;

  try {

    await db.query_trocaprecos("BEGIN");

    await db.query_trocaprecos(`insert into tab_usuario (nom_usuario, senha, schema_base, des_rede, img_rede, ind_aprova_negociacao, ind_ativo)
                                values
                                ($1, $2, $3, $4, $5, $6, 'S')`, [nom_usuario, senha, schema_base, des_rede, img_rede, ind_aprova_negociacao]);
...
exports.sincronizaCadastros = async (req, res) => {
  ...
  try {
    ...
    // Executa a busca de preços e captura o resultado
    const resultadoSP = await db.query_trocaprecos(`
      SELECT zmaisz.sp_busca_preco (
        1,
        ARRAY[0],
        ARRAY[0],
        ARRAY[0],
    ...
    // Verifica o retorno do stored procedure (ajuste conforme o que seu SP retorna)
    const mensagemRetorno = resultadoSP.rows[0]?.resultado || '';
...
exports.removeUsuario = async (req, res) => {
  ...
  try {
    ...
    await db.query_trocaprecos(`update tab_usuario 
                                set ind_ativo = 'N'
                                where cod_usuario = $1
                                and schema_base = $2`, [cod_usuario, schema_base]);
...
exports.buscaUsuario = async (req, res) => {
  ...
  try {
    ...
    const users = await db.query_trocaprecos("select cod_usuario, nom_usuario, ind_aprova_negociacao from tab_usuario where schema_base = $1 and ind_ativo = 'S'", [schema_base]);
...
exports.updateUsuario = async (req, res) => {
  ...
  try {
    ...
    const users = await db.query_trocaprecos("UPDATE tab_usuario set ind_aprova_negociacao = $3 where schema_base = $1 and cod_usuario = $2", [schema_base, cod_usuario, status === true ? 'S' : 'N']);
...
exports.buscaEmpresasBase = async (req, res) => {
  ...
  try {
    ...
    const empresas = await db.query_trocaprecos(`select cod_empresa, nom_fantasia, false as ind_selecionado from ${schema}.tab_empresa_schema where cod_empresa in (${empresa}) order by cod_empresa`);
...
exports.buscaFiltroItem = async (req, res) => {
  ...
  try {
    ...
    //const pessoa = await db.query_trocaprecos(`select cod_pessoa, nom_pessoa, coalesce(num_cnpj_cpf, '') as num_cnpj_cpf , cod_regiao_venda, false as ind_selecionado from ${schema}.tab_pessoa`);
    const formaPagto = await db.query_trocaprecos(`select distinct cod_forma_pagto, des_forma_pagto, false as ind_selecionado, ind_tipo from ${schema}.tab_forma_pagto where cod_empresa in (${cod_empresa}) order by cod_forma_pagto`);
    const itemFull = await db.query_trocaprecos(`select a.cod_item, a.des_item 
                                      from ${schema}.tab_item a
                                      inner join ${schema}.tab_custo_preco b on (a.cod_item = b.cod_item)
                                      inner join ${schema}.tab_item_empresa c on (c.cod_item = a.cod_item and c.cod_empresa = b.cod_empresa) 
...
exports.novaNegociacao = async (req, res) => {
  const { schema, cod_empresa, nom_usuario, cod_usuario, cliente, itens } = req.body;
  ...
  try {
    ...
    novaNegociacaoInsert(schema, cod_empresa, nom_usuario, cod_usuario, cliente, itens);
...
async function novaNegociacaoInsert(schema, cod_empresa, nom_usuario, cod_usuario, cliente, itens) {
  ...
  const seq_lote = await db.query_trocaprecos(`select nextval('${schema}.gen_lote')`);
    ...
    for (const c of cliente) {
      for (let i = 0; i < itens.length; i++) {
        ...
        await db.query_trocaprecos(
          ...
          [
            itens[i].cod_condicao_pagamento, itens[i].cod_empresa, nom_usuario, cod_usuario,
            itens[i].cod_item, c.cod_pessoa, itens[i].dta_inclusao, itens[i].dta_inicio,
            itens[i].ind_percentual_valor, itens[i].ind_tipo_negociacao, itens[i].ind_tipo_preco_base,
            itens[i].val_preco_venda_a, itens[i].val_preco_venda_b, itens[i].val_preco_venda_c,
            itens[i].val_preco_venda_d, itens[i].val_preco_venda_e, "N", "X", `${seq_lote.rows[0].nextval} Inclusao Negociação`
          ]
        );
        ...
        if (progresso % batchSize === 0) {
          // Chama geraStatus
          await geraStatus(seq_lote.rows[0].nextval, total, progresso, "sem erro", empresa, schema);
        }

        if (progresso === total) {
          await geraStatus(seq_lote.rows[0].nextval, total, progresso, "Concluído e Pendente de Aprovação", empresa, schema);
        }
      }
    }
  } catch (error) {
    await geraStatus(seq_lote.rows[0].nextval, null, null, error.message, empresa, schema);
    throw error;
  }
}
...
async function geraStatus(lote, total, progresso, error, empresa, schema) {
  ...
  try {
    ...
    const resgistro = await db.query_trocaprecos(`select * from ${schema}.tab_progresso_lote where seq_lote = $1`, [lote]);

    if (resgistro.rows.length > 0) {
      await db.query_trocaprecos(`update ${schema}.tab_progresso_lote set progresso = $1, error = $3 where seq_lote = $2`, [progresso, lote, error]);
    } else {
      await db.query_trocaprecos(`insert into ${schema}.tab_Progresso_lote ( seq_lote, total, progresso, error )
                                                        values ( $1, $2, $3, $4)`, [lote, total, progresso, "Concluído e Pendente de Aprovação",]);
    }
    ...
    await db.query_trocaprecos("ROLLBACK");
    await db.query_trocaprecos(`update ${schema}.tab_progresso_lote set error = $1 where seq_lote = $2`, [error, lote]);
    throw err;
  }
...
exports.buscaMinhasNegociacoes = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query_trocaprecos(`SELECT a.dta_inclusao, a.seq_lote_alteracao, b.nom_fantasia, b.cod_empresa, a.ind_excluido, COUNT(*) AS total_registros, c.progresso, c.total, c.error, a.des_observacao, a.ind_status
                                          ...
                                          group by 1,3,2,4,5,7,8,9,10,11`, [cod_usuario]);
...
exports.buscaNegociacoesEmpresa = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query_trocaprecos(`SELECT a.dta_inclusao, a.seq_lote_alteracao, b.nom_fantasia, b.cod_empresa, a.ind_excluido, COUNT(*) AS total_registros, c.progresso, c.total, c.error, a.des_observacao
                                          FROM ${schema}.tab_nova_regra a
                                          inner join ${schema}.tab_empresa_schema b on (a.cod_empresa = b.cod_empresa)
                                          inner join ${schema}.tab_progresso_lote c on (a.seq_lote_alteracao = c.seq_lote)
...
exports.buscaMinhasNegociacoesDetalhe = async (req, res) => {
  ...
  const { schema, cod_usuario, cod_empresa, seq_lote_alteracao, ind_aprovacao } = req.body;
  ...
  try {
    ...
    if (ind_aprovacao === 'S') {
      ...
      const result = await db.query_trocaprecos(`select distinct a.seq_lote_alteracao, a.cod_condicao_pagamento, 
                                                  CASE
                                                  ...
                                                  and a.cod_empresa in (${cod_empresa})
                                                  order by b.nom_pessoa, d.des_item`, [seq_lote_alteracao]);
      ...
      const result = await db.query_trocaprecos(`select distinct a.seq_lote_alteracao, a.cod_condicao_pagamento, 
                                    ...
                                    and a.cod_usuario = $2
                                    and a.cod_empresa in (${cod_empresa})
                                    order by b.nom_pessoa, d.des_item`, [seq_lote_alteracao, cod_usuario]);
...
exports.buscaAtualizacaoNegociacao = async (req, res) => {
  ...
  const { schema, cod_usuario, cod_empresa, item, formaPagto, pessoa } = req.body;
  ...
  try {
    ...
    const result = await db.query_trocaprecos(`select distinct a.cod_condicao_pagamento, c.des_forma_pagto, a.cod_item, d.des_item, a.cod_pessoa, b.nom_pessoa, a.dta_inclusao, a.dta_inicio, false as ind_alterado,
...
exports.atualizaNegociacao = async (req, res) => {
  ...
  try {
    ...
    novaAtualizaNegociacao(schema, cod_empresa, nom_usuario, cod_usuario, cliente, itens);
...
async function novaAtualizaNegociacao(schema, cod_empresa, nom_usuario, cod_usuario, itens) {
  ...
  try {
    ...
    for (let i = 0; i < itens.length; i++) {
      ...
      await db.query_trocaprecos(
        ...
        [
          itens[i].cod_condicao_pagamento, nom_usuario, cod_usuario,
          itens[i].cod_item, itens[i].cod_pessoa, itens[i].dta_inclusao, itens[i].dta_inicio,
          itens[i].ind_percentual_valor, itens[i].ind_tipo_negociacao, itens[i].ind_tipo_preco_base,
          itens[i].new_val_preco_venda_a, itens[i].new_val_preco_venda_b, itens[i].new_val_preco_venda_c,
          itens[i].new_val_preco_venda_d, itens[i].new_val_preco_venda_e, "N", "X", `${seq_lote.rows[0].nextval} Atualizacao Negociação`
        ]
      );
...
exports.excluirNegociacao = async (req, res) => {
  ...
  try {
    ...
    await db.query_trocaprecos(`update ${schema}.tab_nova_regra
                    set ind_excluido = 'S',
                    ind_status = 'U'
                    where cod_empresa in ($1)
                    and seq_lote_alteracao = $2`, [cod_empresa, seq_lote]);
  ...
  try {
    ...
    for (const i of item) {
      ...
      await db.query_trocaprecos(
        ...
        [
          null, i.cod_empresa, nom_usuario, cod_usuario,
          i.cod_item, null, dataDeHoje.format("YYYY-MM-DD"), dataDeHoje.format("YYYY-MM-DD"),
          "V", "P", "A", i.val_novo_preco_venda, 0, 0, 0, 0, "N", "X", `${seq_lote.rows[0].nextval} Troca de Preços Bomba`
        ]
      );
      progresso++;

      if (progresso === total) {
        await geraStatus(seq_lote.rows[0].nextval, total, progresso, "concluído", 0, schema);
      }
    }
    //await db.query_trocaprecos(`update ${schema}.tab_nova_regra set ind_status = 'T' where seq_lote_alteracao = ${seq_lote.rows[0].nextval} `);
    await db.query_trocaprecos("COMMIT");
    ...
    await geraStatus(seq_lote.rows[0].nextval, null, null, error.message, 0, schema);
    throw error;
  }
...
exports.aprovaRegra = async (req, res) => {
  ...
  try {
    ...
    await db.query_trocaprecos(`update ${schema}.tab_nova_regra
                    ...
                    and seq_lote_alteracao = $1`, [seq_lote, nom_usuario, cod_empresa]);

    await db.query_trocaprecos(`update ${schema}.tab_progresso_lote set error = 'Concluído e Aprovado'`);
...
exports.buscaPrecoIntervalo = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query_trocaprecos(`SELECT distinct
                                                a.cod_item, 
                                                a.dta_inicio, 
                                                a.val_preco_venda_a, 
                                                a.val_preco_venda_b, 
                                              ...
                                              and a.val_preco_venda_a < $2`, [precoInicial, precoFinal]);
