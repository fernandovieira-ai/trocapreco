...
exports.listaContaBanco = async (req, res) =>{
  ...
  try {
    ...
    const result = await db.queryGaragem("select seq_conta, des_conta, num_conta, false as ind_selecionado, ind_cc_investidor from tab_conta_banco where ind_cc_investidor = false");
...
exports.lancaDespesaVeiculo = async (req, res) =>{
  ...
  try {
    ...
    await db.queryGaragem(`insert into tab_despesa_veiculo (
      ...
      $4,
      $5
      )`,[val_despesa, des_origem, dta_despesa, banco, des_despesa]);  
...
exports.veiculosVendidos = async (req, res) =>{
  ...
  try {
    ...
    const result = await db.queryGaragem(`select 
                                    ...
                                    order by a.dta_venda desc limit 50`);
...
exports.listaLucroOperacao = async ( req, res ) =>{
  ...
  try {
    ...
    const lucroCarro = await db.queryGaragem(`select 
                                        ...
                                        from tab_veiculo
                                        where ind_status = 'V'
                                        AND ind_tipo_veiculo = 'P' order by dta_venda desc`);
...
exports.metricaVendaVeiculo = async (req, res) =>{
  ...
  try {
    ...
    const vendaPropria = await db.queryGaragem(`SELECT
                                          EXTRACT(MONTH FROM dta_venda) AS mes,
                                          COUNT(*) AS contagem,
                                          CASE
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 1 THEN 'Janeiro'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 2 THEN 'Fevereiro'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 3 THEN 'Março'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 4 THEN 'Abril'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 5 THEN 'Maio'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 6 THEN 'Junho'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 7 THEN 'Julho'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 8 THEN 'Agosto'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 9 THEN 'Setembro'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 10 THEN 'Outubro'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 11 THEN 'Novembro'
                                              WHEN EXTRACT(MONTH FROM dta_venda) = 12 THEN 'Dezembro'
                                          END AS nome_mes
                                          FROM tab_veiculo
                                          WHERE ind_status = 'V'
                                          and ind_tipo_veiculo = 'P'
                                          and dta_venda between $1 and $2
                                          GROUP BY mes, ind_tipo_veiculo
                                          ORDER BY mes;`,[dta_inicial, dta_final]);

    const vendaTerceiros = await db.queryGaragem(`SELECT
                                                EXTRACT(MONTH FROM dta_venda) AS mes,
                                                COUNT(*) AS contagem,
                                                CASE
                                                    WHEN EXTRACT(MONTH FROM dta_venda) = 1 THEN 'Janeiro'
    ...
    const vendaInvestidor = await db.queryGaragem(`WITH meses AS (
                                                ...
                                                SELECT
                                                    m.mes,
                                                    COALESCE(COUNT(v.dta_venda), 0) AS contagem,
                                                    CASE
                                                    ...
                                                    END AS nome_mes
                                                FROM meses m
                                                LEFT JOIN tab_veiculo v ON EXTRACT(MONTH FROM v.dta_venda) = m.mes
                                                    AND v.ind_status = 'V'
                                                    AND v.dta_venda BETWEEN $1 AND $2
                                                    AND v.ind_tipo_veiculo = 'I'
                                                GROUP BY m.mes
                                                ORDER BY m.mes;`,[dta_inicial, dta_final]);

    const mesesDeVenda = await db.queryGaragem(`SELECT
                                                EXTRACT(MONTH FROM dta_venda) AS mes,
                                                CASE
                                                    WHEN EXTRACT(MONTH FROM dta_venda) = 1 THEN 'Janeiro'
                                                    WHEN EXTRACT(MONTH FROM dta_venda) = 2 THEN 'Fevereiro'
...
exports.metricaVendaVendedor = async (req, res) =>{
  ...
  try {
    ...
    // Criar um array de objetos que contém o mês e as datas inicial e final desse mês
    const meses = [];
    for (let i = 1; i <= currentMonth; i++) {
      meses.push({
        mes: i,
        dta_inicial: moment(`${currentYear}-${i}`, 'YYYY-MM').startOf('month').format('YYYY-MM-DD'),
        dta_final: moment(`${currentYear}-${i}`, 'YYYY-MM').endOf('month').format('YYYY-MM-DD')
      });
    }
