...
export class AuthService {
  ...
  constructor(private router: Router, public socket: WebsocketService, private httpClient: HttpClient, private platform: Platform) { }
  ...
  isTokenExpired(token: string): boolean {
    if (!token) { //se nao tem token
      return true;
    }

    const date = this.getAuthTokenExpireIn(token);
    if (date === undefined) { // se nao tem data
      return false;
    }
  ...
  isLogged() {
    ...
    if (!token) { // se nao tem token
      return false;
    } else if (this.isTokenExpired(token)) { //se tem token, valido se esta expirado
      return false;
    }
